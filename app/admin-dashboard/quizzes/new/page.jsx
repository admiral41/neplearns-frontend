...
export default function CreateQuizPage() {
    ...
    const fetchWeeks = async (courseId) => {
        ...
        try {
            ...
            // Clear week and lesson selections
            setFormData(prev => ({ ...prev, week: "", lesson: "" }));
            setLessons([]);
        } catch (error) {
    ...
    const saveQuestion = () => {
        ...
        if (questionForm.questionType === 'single_choice' || 
            ...
            // Get correct answers from selected options
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // TEMPORARY FIX: Remove week and lesson until we debug the hierarchy
            const quizData = {
                title: formData.title.trim(),
                description: formData.description?.trim() || "",
                instructions: formData.instructions?.trim() || "",
                course: formData.course,
                // Temporarily remove week and lesson
            ...
            if (!createResponse.success) {
                throw new Error(createResponse.msg || 'Failed to create quiz');
            }

            const quizId = createResponse.data._id;
            let createdQuestions = 0;

            // Step 2: Add questions if there are any
            if (questions.length > 0) {
                for (const [index, question] of questions.entries()) {
                    try {
                        const questionData = {
                            ...
                            tags: question.tags,
                            order: index,
                            // Add options and correctAnswers based on question type
                            ...(question.options && { options: question.options }),
                            ...(question.correctAnswers && { correctAnswers: question.correctAnswers })
                        };

                        const questionResponse = await quizAPI.addQuestion(quizId, questionData);
            ...
            toast.success(`Quiz created successfully with ${createdQuestions} questions!`);
            ...
            toast.error(error.response?.data?.msg || error.message || 'Failed to create quiz');
        } finally {
        ...
        <AdminDashboardLayout>
            <div className="px-4 py-6 sm:py-8">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-lg sm:text-xl font-bold">Create New Quiz</h1>
                            <p className="text-sm text-muted-foreground">
                                Design and configure a new quiz for your course
                            </p>
                        </div>
                    </div>
                                ...
                                <Card>
                                    ...
                                    <CardContent className="space-y-4">
                                        ...
                                        <div className="grid sm:grid-cols-3 gap-4">
                                            ...
                                            {/* Temporarily comment out week and lesson selection */}
                                            {/* <div className="space-y-2">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-sm text-muted-foreground truncate">
                                                {/* Temporarily remove week and lesson info */}
                                                Course Only
                                            </p>
                                        </div>
                                    </div>
                            ...
                            <CardContent className="space-y-3">
                                <Button
                                    ...
                                    onClick={() => {
                                        ...
                                        link.href = url;
                                        link.download = `quiz-questions-${Date.now()}.json`;
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        const quizData = {
                                            ...formData,
                                            title: `${formData.title || "New Quiz"} (Copy)`,
                                            isPublished: false
                                        };
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        if (validateForm()) {
                                            toast.success("Quiz is valid and ready!");
                                        }
                                    }}
                                >
                                    <CheckCircle className="h-4 w-4 mr-2" />
            ...
            <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>
                            {formData.isPublished ? "Publish Quiz" : "Save as Draft"}
                        </AlertDialogTitle>
                        <AlertDialogDescription>
                            {formData.isPublished ? (
                                <>
                                    This quiz will be published and available to students immediately.
                                    ...
                                    <div className="mt-4 space-y-2">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <CheckCircle className="h-4 w-4 text-green-500" />
                                            <span className="text-sm">Duration: {formData.duration} minutes</span>
                                        </div>
                                    </div>
                                </>
                            ) : (
                                "This quiz will be saved as a draft and won't be visible to students."
                            )}
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        ...
                        >
                            {isSubmitting ? (
                                ...
                                formData.isPublished ? "Publish Quiz" : "Save Draft"
                            )}
                        </AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
